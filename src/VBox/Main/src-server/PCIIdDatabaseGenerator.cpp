/* $Id: PCIIdDatabaseGenerator.cpp 113422 2026-03-16 14:28:47Z alexander.eichner@oracle.com $ */
/** @file
 * PCI device vendor and product ID database - generator.
 */
...
#include "../include/PCIIdDatabase.h"


/*
 * Include the string table generator.
 */
#define BLDPROG_STRTAB_MAX_STRLEN  (PCI_ID_DATABASE_MAX_STRING - 1)
#ifdef PCI_ID_DATABASE_WITH_COMPRESSION
        ...
        || vrc == VINF_SUCCESS)
    {
        ...
        if (*pszNext != '\0')
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (cchDesc <= PCI_ID_DATABASE_MAX_STRING)
                {
                    id   = uVal;
                    desc = pszNext;
...
static int ParsePciIds(PRTSTREAM pInStrm, const char *pszFile)
{
    /*
     * State data.
     */
    ...
    for (;;)
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            else if (szLine[0] == '\t' && szLine[1] == '\t') /** @todo Ignore subsystem vendor and product for now. */
                continue;
...
static void WriteSourceFile(FILE *pOut, const char *argv0, PBLDPROGSTRTAB pStrTab)
{
    fprintf(pOut,
            "/** @file\n"
            " * PCI device vendor and product ID database - Autogenerated by %s\n"
            " */\n"
            "\n"
            "/*\n"
            " * Copyright (C) 2026-" VBOX_C_YEAR " Oracle and/or its affiliates.\n"
            " *\n"
            " * This file is part of VirtualBox base platform packages, as\n"
            " * available from https://www.virtualbox.org.\n"
            " *\n"
    ...
    BldProgStrTab_WriteStringTable(pStrTab, pOut, "", "PCIIdDatabase::s_", "StrTab");

    fputs("/**\n"
          " * PCI devices aliases array.\n"
          " * Format: VendorId, ProductId, Vendor Name, Product Name\n"
          " * The source of the list is https://github.com/pciutils/pciids/blob/master/pci.ids\n"
          " */\n"
          "PCIIDDBPROD const PCIIdDatabase::s_aProducts[] =\n"
          "{\n", pOut);
    ...
    fputs("};\n"
          "\n"
          "\n"
          "const RTBLDPROGSTRREF PCIIdDatabase::s_aProductNames[] =\n"
          "{\n", pOut);
    for (ProductsSet::iterator itp = g_products.begin(); itp != g_products.end(); ++itp)
        fprintf(pOut, "{ 0x%06x, 0x%02x },\n", itp->StrRef.offStrTab, (unsigned)itp->StrRef.cchString);
    fputs("};\n"
          "\n"
          "const size_t PCIIdDatabase::s_cProducts = RT_ELEMENTS(PCIIdDatabase::s_aProducts);\n"
          "\n", pOut);

    fputs("PCIIDDBVENDOR const PCIIdDatabase::s_aVendors[] =\n"
          "{\n", pOut);
    ...
    fputs("};\n"
          "\n"
          "\n"
          "const RTBLDPROGSTRREF PCIIdDatabase::s_aVendorNames[] =\n"
          "{\n", pOut);
    for (VendorsSet::iterator itv = g_vendors.begin(); itv != g_vendors.end(); ++itv)
        fprintf(pOut, "{ 0x%06x, 0x%02x },\n", itv->StrRef.offStrTab, (unsigned)itv->StrRef.cchString);
    fputs("};\n"
          "\n"
          "const size_t PCIIdDatabase::s_cVendors = RT_ELEMENTS(PCIIdDatabase::s_aVendors);\n"
          "\n", pOut);
}

static int usage(FILE *pOut, const char *argv0)
{
    fprintf(pOut, "Usage: %s [https://github.com/pciutils/pciids/blob/master/pci.ids] [custom pci list file] [-o output file]\n", argv0);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        vrc = ParsePciIds(pInStrm, argv[i]);
        RTStrmClose(pInStrm);
        if (vrc != 0)
        {
            RTMsgError("Failed parsing PCI devices file '%s'", argv[i]);
            return vrc;
        }
    }

    /*
     * Due to PCIIDDBVENDOR::iProduct, there is currently a max of 64KB products.
     ...
     */
    size_t const cbVendorEntry  = sizeof(PCIIdDatabase::s_aVendors[0]) + sizeof(PCIIdDatabase::s_aVendorNames[0]);
    size_t const cbProductEntry = sizeof(PCIIdDatabase::s_aProducts[0]) + sizeof(PCIIdDatabase::s_aProductNames[0]);
