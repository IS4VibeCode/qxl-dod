...
#print "********* START *********\n";                                           #debug
#print "args:";                                                                 #debug
#foreach my $arg (%args) {                                                      #debug
#  print " $arg";                                                               #debug
#}                                                                              #debug
#print "\n";                                                                    #debug
#foreach my $bld_dir (@build_dirs) {                                            #debug
#  print "buildir: $bld_dir\n"                                                  #debug
#}                                                                              #debug
#print "now: $current_time  start: $start_seconds  end: $end_seconds\n";        #debug
...
#      print "add:  $build_stat[9] - $build\n";                                 #debug
    }
#    else { print "skip: $build_stat[9] - $build\n"; }                          #debug
  }
...
#my $total_size_start = $total_size;                                            #debug
#print "total_size: " . ($total_size / (1024*1024)) . " ($total_size)  quota: $args{quota} (" . ($args{quota}*1024*1024) . ")\n";       #debug
...
#my $deleted = $total_size_start - $total_size;                                     #debug
#print "total_size: " . ($total_size / (1024*1024)) . " ($total_size)  deleted: ". ($deleted / (1024*1024)) ." ($deleted)\n";   #debug
...
sub delete_build {
  ...
  if ($tbox_url) {
    ...
    die "Could not delete $build_url via $tbox_url/xml/build_deleted.pl: " . $res->status_line . "\n" if !$res->is_success();
  }
