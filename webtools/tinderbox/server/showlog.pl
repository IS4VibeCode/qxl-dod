...
if ($p->param('format') && $p->param('format') eq 'raw') {
  ...
  if ($title eq "Summary") {
    ...
    while (($pos = tell($log_fh)) != -1 && ($_ = readline($log_fh))) {
      ...
      if (/^--->/ || /^<---/ || /: \*\*\* /) {
        ...
        print " " x ($tab*5);
        if (/: \*\*\*/) {
            # Drop tedious stuff kbuild source reference: kmk: *** [/Users/vbox/tinderbox/mac4-chk/kBuild/footer-pass2-compiling-targets.kmk:277: ...]
            s/\[[^ ]*footer-pass2-compiling-targets.kmk:\d+: */[/;
        }
        my $escaped = $_;
        $escaped =~ s/&/&amp;/g;
        $escaped =~ s/</&lt;/g;
  ...
  while ($pos <= $page * $bytes_per_page && ($buf = readline($log_fh))) {
    if (index($buf,'<') >= 0 or index($buf,'>') >= 0 or index($buf,'&') >= 0) {
      $buf =~ s/&/&amp;/g;
      $buf =~ s/</&lt;/g;
      $buf =~ s/>/&gt;/g;
    }
  ...
  print "<p>Load time: $load_time ms</p>";
}
