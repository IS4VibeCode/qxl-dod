...
print "<table class=editlist><tr><th colspan='2'>Trees</th></tr>\n";
my $sth = $dbh->prepare("SELECT tree_name FROM tbox_tree ORDER BY tree_name");
$sth->execute();
while (my $tree_info = $sth->fetchrow_arrayref()) {
  ...
  print "<tr><td><a href='showbuilds.pl?tree=$tree'>$tree</a></td><td>" . join(' | ', @actions) . "</td></tr>\n";
}

print "<tr><td colspan='2'><a href='admintree.pl'>Add Tree</a></td></tr>\n";
print "</table>\n";
