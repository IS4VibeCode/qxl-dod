...
sub _grab_cache {
  ...
  if ($bonsai_url !~ m/^http/) {
    if ($bonsai_url =~ m/^\//) {
      $bonsai_url = "http://localhost$bonsai_url";
    } else {
      $bonsai_url = "http://localhost$bonsai_url";
    }
  }
...
\s*(?:</\w+[^>]*>\s*)*)?
      ...
      my $was_description = defined($9) || $prev_checkin_date ne "$1 $2";
      if ($was_description) {
        ...
        ($checkin_date, $who, $revisions, $size_plus, $size_minus) = ("$1 $2", $3, $5, $7, $8);
        $description = $9 if (defined($9));
      } else {
        $revisions .= ",$5";
        $size_plus += $7;
        $size_minus += $8;
      }
      $prev_checkin_date = "$1 $2";
