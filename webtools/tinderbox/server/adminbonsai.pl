...
<tr><th>Module (vbox: svn repository):</th><td>@{[$p->textfield(-name=>'module', -default=>$module)]}</td></tr>
...
<tr><th>cvsroot (vbox: use 'subversion'):</th><td>@{[$p->textfield(-name=>'cvsroot', -default=>$cvsroot)]}</td></th></tr>
</table>
<p><b>PLEASE NOTE that pressing submit will clear the Bonsai cache for this tree.  This is not disastrous by any means, but don't submit unless you are actually changing or adding this tree.</p>
<p>VBox Note! Putting 'subversion' in the cvsroot field is a hack to piggy-back on the TestManager revision history. The module
              field will be use as the subversion repository. (Requires foreign table like with the users.)</p>
EOM
