...
# $HeadURL$
# $LastChangedDate: 2026-05-05 09:55:34 +0200 (Tue, 05 May 2026) $
# $LastChangedBy: alexander.eichner@oracle.com $
# $LastChangedRevision: 114067 $
...
# Whether to apply the -p file to existing files too.
my $opt_fix_existing_properties = 0;
...
GetOptions('no_user_input'           => \$opt_no_user_input,
           ...
           'fix_existing_properties' => \$opt_fix_existing_properties,
                    ...
                    if (@add_dirs)
                      {
                        ...
                        # VBox: export the new directories.
                        read_from_process($svn, 'propset', '--force', 'svn:sync-process', 'export', @add_dirs);
                      }
            ...
            if (defined (my $del_digest = $del_info->{digest}))
              {
                ...
                if ($new_digest ne $del_digest  
                    or (    $opt_fix_existing_properties != 0 
                        and @property_settings
                        and $source_type eq 'f'))
                  {
                    print "U   $source_path\n";
    ...
    if (keys %add_files)
      {
            ...
            foreach my $property (@{$add_files{$add_file}{properties}})
              {
                ...
                if ($property_name eq 'svn:eol-style' and $property_value ne '')
                  {
                    @diff_ignore_space_changes = ('-b');

                    # Must convert line endings before applying eol-style.
                    if (&convert_file_to_given_eol($add_file, $property_value))
                      {
                        print "eol-style=$property_value conversion modified $add_file.\n";
                      }
                  }
        ...
        # VBox: export the added files.
        $target_filename = &make_targets_file(@add_files);
        read_from_process($svn, 'propset', '--targets', $target_filename, '--force', 'svn:sync-process', 'export');
        unlink($target_filename);
      }
    if (@del_files)
      {
    ...
    if (keys %upd_files)
      {
        ...
        foreach my $upd_file (@upd_files)
          {
            # Fix properties.  See comments in $wanted for details on how this works.
            if ($opt_fix_existing_properties != 0 and @property_settings)
              {
                foreach my $property (@property_settings)
                  {
                    my $re = $property->{re};
                    if ($upd_file =~ $re)
                      {
                        ...
                        if (    defined $property_name 
                            and defined $property_value
                            and $property_name ne 'svn:externals' 
                            and $property_name ne 'svn:ignore' )
                          {
                            if ($property_name eq 'svn:eol-style' and $property_value ne '')
                              {
                                if (&convert_file_to_given_eol($upd_file, $property_value))
                                  {
                                    print "eol-style=$property_value conversion modified $upd_file.\n";
                                  }
                              }
                            print "Adding to '$upd_file' property '$property_name' with value '$property_value'.\n";
                            read_from_process($svn, 'propset', $property_name, $property_value, "$upd_file");
                          }
                        last if $property->{control} eq 'break';
                      }
                  }
            ...
            # else svn 1.9 will error when it is not found, see:
            #     https://issues.apache.org/jira/browse/SVN-4621
            my @proplist = read_from_process($svn,'proplist','--quiet',"$upd_file\@BASE");
            ...
            if ($eol_style ne '')
              {
                @diff_ignore_space_changes = ('-b');
                if (&convert_file_to_given_eol($upd_file, $eol_style))
                  {
                    print "eol-style=$eol_style conversion modified $upd_file.\n";
                  }
              }
          }
      }
...
sub usage
{
  ...
  die "usage: $0 [options] svn_url svn_import_dir [dir_v1 [dir_v2 [..]]]\n",
      ...
      "  -fix_existing_properties  Apply the -p option file to existing files\n",
...
sub read_from_process
{
  ...
  if ($status)
    {
      # propget hack.
      #print STDERR "debug: status: $status\n";
      if ($status == 1 or $status == 256)
      {
          #print STDERR "debug: status check: fine\n";
          if (@output >= 1) 
          {
              #print STDERR "debug: output line check: fine\n";
              #print STDERR "debug: first output line:", $output[0], "\n";
              if (index($output[0], " W200017: Property") != -1)
              {
                  @output = ();
                  return @output;
              }
...
# Take a filename, open the file and replace all CR, CRLF, CRCRLF and LF's
# with the given EOL sequence, stripping trailing spaces.
sub convert_file_to_given_eol
{
  unless (@_ == 2)
    {
      croak "$0: convert_file_to_given_eol $INCORRECT_NUMBER_OF_ARGS";
    }

  my $filename = shift;
  my $eol_seq = shift;

  if ($eol_seq eq "native")     { $eol_seq = $native_eol; }
  elsif ($eol_seq eq "LF")      { $eol_seq = "\012"; }
  elsif ($eol_seq eq "CRLF")    { $eol_seq = "\015\012"; }
  else                          { die "$0: unknown eol-style='$eol_seq'"; }
      ...
      if ($c eq "\012" or $c eq "\015")
        {
          if ($c eq "\015")
            {
              my $c2 = substr($in, $i + 1, 1);
              # \r\n:
              if ($c2 eq "\012")
                {
                  ++$i;
                }
              # \r\r\n: Seen a lot in UDK2014.SP1 copyright notices (e.g. Vlv2TbltDevicePkg/AcpiPlatform/AcpiPlatform.h).
              elsif ($c2 eq "\015" and substr($in, $i + 2, 1) eq "\012")
                {
                  $i += 2;
                }
            }

          # Strip trailing spaces and tabs before adding the native eol.
          $c = substr($out, -1);
          while ($c eq " " or $c eq "\t")
          {
              chop($out);
