NSIS (Nullsoft Installation Script) 3.10 (1st respin)
...
- Do a x86 release build: kmk KBUILD_TARGET_ARCH=x86 KBUILD_TYPE=release
...
- run: ren nsis-3.10-src nsis-3.10-r1
- run: cd nsis-3.10-r1
- Apply the patch attached to this file: 
    - run: patch -p1 < ..\readme.tool
- run: kmk_redirect -A "Path=;%MY_VBOX_ROOT%\tools\win.x86\HTML_Help_Workshop\v1.3" -E SCONS_MSCOMMON_DEBUG=%CD%\scons-mscommon.log ^
        -- cmd.exe /c C:\Python312\scripts\scons.exe MSVC_USE_SCRIPT=None MSTOOLKIT=yes MSVS_VERSION=14.3 ^
        UNICODE=yes SKIPUTILS="NSIS Menu" SKIPTESTS=all APPEND_CCFLAGS=-arch:IA32 STRIP=1 STRIP_W32=1 NSIS_CONFIG_LOG=1 ^
        ZLIB_W32=%MY_VBOX_ROOT%/out/win.x86/release/nsis-zlib/ dist |& tee bld-1.log
- run: cd .instdist
- run: copy ..\..\readme.tool
- Check that OS and subsys versions are 4.0:
    - run: dumpbin /HEADERS Plugins\x86-unicode\nsExec.dll | grep -i version
    - run: dumpbin /HEADERS Stubs\zlib-x86-unicode | grep -i version
- run: zip -r9X ..\..\win.x86.nsis.v3.10-log-r1.zip * -x "*ansi*" -x makensisw.exe -x NSIS.chm -x Examples\* -x Docs\*
...
- The -arch:AI32 compiler option is to stop the compiler from emitting SSE code
  which will make it impossible to run the installer on NT4.
- We only need x86 NSIS at the moment, since both amd64 and arm64 can execute
  x86 binaries.  (If we want to emit native installers later, we could merge the
  amd64 and arm64 stubs and plugins into the x86 package, and then use the
  Target directive to pick what to use.)
...
+		oAction = defenv.Action(sVBoxSetPeVersion + ' --nt4 "%s"' % (oTarget.path,))
