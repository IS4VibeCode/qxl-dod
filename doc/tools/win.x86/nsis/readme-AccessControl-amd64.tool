...
- run: SET MY_VBOX_PE_SET_VERSION=%MY_VBOX_ROOT%\out\win.amd64\release\obj\VBoxPeSetVersion\VBoxPeSetVersion.exe
...
- All the above is the same as for the amd64 NSIS package.
...
- run: mkdir accesscontrol-amd64
- run: cd accesscontrol-amd64
...
- run: mkdir Plugins Plugins\amd64-unicode build build\amd64-unicode
- run: cd build\amd64-unicode
- run: kmk_ln -s %MY_NSIS_SRC%\Contrib\ExDLL\ nsis_unicode
- run: cl /Zi /LD /W3 /O1 /GF /Gz /GS- /GR- /GL /Zl /Osy /Ogsy /DUNICODE /D_UNICODE ^
        /I../../Contrib/AccessControl ^
        /I%MY_NSIS_SRC%\Source\exehead /I%MY_NSIS_SRC%\Contrib\ExDLL /I. ^
        ../../Contrib/AccessControl/AccessControl.cpp ^
        %MY_NSIS_SRC%\Contrib\ExDLL\pluginapi.c ^
        /FeAccessControl.dll /Fm ^
        /link /OPT:REF /OPT:ICF,99 /MERGE:.rdata=.text /MAP /NODEFAULTLIB kernel32.lib user32.lib advapi32.lib
- run: %MY_VBOX_PE_SET_VERSION% --xp64 AccessControl.dll
- run: RTLdrCheckImports -p %MY_VBOX_ROOT%\tools\win.amd64\exports\xp64 AccessControl.dll
- run: copy AccessControl.dll ..\..\Plugins\amd64-unicode
- run: copy AccessControl.pdb ..\..\Plugins\amd64-unicode
- run: cd ..\..
- run: copy ..\readme-AccessControl-amd64.tool
- run: zip -9Xr ..\win.amd64.nsis.v3.10-log-AccessControl-v1.0.8.3.zip Plugins Docs readme-AccessControl-amd64.tool
